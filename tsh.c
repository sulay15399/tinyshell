...
void do_bgfg(char **argv) 
{
	...
	if (argv != NULL)
	{
		...
		char *cmd;	// store string
		int cmd_type = strcmp(argv[0], "fg");	// determine value
		if (cmd_type == 0)
			cmd = "fg";
		else
			cmd = "bg";
...
void sigchld_handler(int sig) 
{
	...
	while((pid=Waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
	{
		...
		{  
			// delete from jobs and print status
			deletejob(jobs,pid);			
...
void sigint_handler(int sig)
{
	...
	// check status of fg_pid
	if (fg_pid)
	{
		// signal on positive check
		Kill((-1)*(fg_pid),sig);
	}
	return;
}
